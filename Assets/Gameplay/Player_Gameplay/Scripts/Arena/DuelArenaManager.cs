...
public class DuelArenaManager : MonoBehaviour
{
    ...
    public bool TryStartDuel(ulong playerA, ulong playerB, out int duelId)
    {
        ...
        Vector3 center = (originalPosA + originalPosB) * 0.5f;
        ...
        teleportA.EnterDuelMode(duelPosA, duelRotA, arenaWorldPosition, arenaWorldRotation, playerAName, playerBName);
        teleportB.EnterDuelMode(duelPosB, duelRotB, arenaWorldPosition, arenaWorldRotation, playerBName, playerAName);

        Debug.Log($"Duelo iniciado. SessionId={duelId}, Slot={slotIndex}");
